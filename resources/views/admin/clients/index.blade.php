                    ...
                    @foreach($clients as $client)
                        <tr class="align-middle">
                            ...
                            <td class="d-flex align-items-center">
                                @switch($client->status)
                                    @case(1)
                                        <a href="{{ route('admin.clients.status', $client) }}"
                                           class="btn btn-warning text-white d-inline-block me-2">Inactive</a>
                                        @break
                                    @case(0)
                                        <a href="{{ route('admin.clients.status', $client) }}"
                                           class="btn btn-success text-white d-inline-block me-2">Active</a>
                                        @break
                                @endswitch
